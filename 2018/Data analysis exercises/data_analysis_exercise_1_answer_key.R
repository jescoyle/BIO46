...
# Create a histogram with bins of width 50 mg
lichens_grw %>%
  ggplot(aes(x = Mass_mg_init)) +
...
# Create a histogram of the number of ascomata
lichens_grw %>%
  ggplot(aes(x = Ascomata)) +
  geom_histogram(binwidth = 1) +
  labs(x = "Number of ascomata",
       y = "Number of lichens"
  )
...
# Create a scatterplot with line with 75% CI
lichens_grw %>%
  ggplot(aes(x = Mass_mg_init, y = Ascomata)) +
  geom_point() +
  geom_smooth(method = "lm",
