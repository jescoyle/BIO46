...
**YOUR TURN**: Add a column to `my_data` called `temp_F` which is the Fahrenheit equivalent of the Celcius temperature displayed in `temp_C`. Recall that you can use `my_data$new_column_name = ` to add on a column called 'new\_column\_name' to a dataframe.
